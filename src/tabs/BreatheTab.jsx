...
export default function BreatheTab() {
  ...
  return (
    <div style={{ paddingTop:12, animation:"fadeIn 0.4s ease" }}>
      ...
      {/* Technique picker — only shown when not active */}
      {!breathActive && (
        <div style={{ marginBottom:18 }}>
          <p style={{ fontFamily:F.b, fontWeight:700, fontSize:12, color:C.muted,
            letterSpacing:1.2, textTransform:"uppercase", marginBottom:10, textAlign:"center" }}>
            Choose a technique
          </p>
            ...
            {TECHNIQUES.map((t,i)=>(
              <button key={t.id} onClick={()=>{ setTechIdx(i); setBreathPhase(0); }} style={{
                ...
                <div style={{ fontFamily:F.b, fontWeight:500, fontSize:11,
                  color: techIdx===i ? "rgba(255,255,255,0.75)" : C.muted,
                  lineHeight:1.4 }}>{t.desc}</div>
              </button>
            ))}
          </div>
        </div>
